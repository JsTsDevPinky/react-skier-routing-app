  ...
  while (previousPoint !== start) {
    ...
    for (const neighbor in neighbors) {
      ...
      neighborRoutes.forEach((routeId) => {
        ...
        const expectedDistance = distances[previousPoint] - routeInfo.length;
        if (distanceToNeighbor === expectedDistance) {
          shortestPath.unshift(neighbor);
          usedRoutes[`${neighbor}-${previousPoint}`] = routeInfo; // Store route in reverse order
          previousPoint = neighbor;
          console.log(
            `Added route: ${neighbor} -> ${previousPoint}, routeId: ${routeInfo.id}`
          );
        }
      });
    }
...
function findEasiestRoute(start, end, graph, routes) {
  ...
  let previousPoint = end;
  while (previousPoint !== start) {
    const { from } = usedRoutes[previousPoint];
    easiestPath.unshift(from);
    previousPoint = from;
  }

  // Generate the easiest routes array
  const easiestRoutes = [];
  for (let i = 0; i < easiestPath.length - 1; i++) {
    const from = easiestPath[i];
    const to = easiestPath[i + 1];
    const route = usedRoutes[to];
    easiestRoutes.push(routes.find((r) => r.id === route.routeId));
  }
...
// Function to find the fastest route between two points using Dijkstra's algorithm
function findFastestRoute(start, end, graph, routes) {
  ...
  const shortestRoutes = shortestPath.slice(1).map((point, index) => {
